...
btc_chg = spot.get("BTC", {}).get("chg")
label, color, advice = C.regime(dur["index"], hor["index"], btc_chg)
...
# THE ONE RULE banner
if fade:
    st.error(f"🚫 **FADE SIGNAL** — {fade_msg}", icon="🚫")
else:
...
with c1:
    ...
    fig.update_layout(height=200, margin=dict(t=40, b=5, l=25, r=25))
...
with c2:
    ...
    fig2.update_layout(height=200, margin=dict(t=40, b=5, l=25, r=25))
...
def yld(key):
    ...
    return (s.iloc[-1] / 10, (s.iloc[-1] - s.iloc[-2]) * 10) if len(s) > 1 else (np.nan, np.nan)


v, d = yld("US30Y")
k[0].metric("30Y ⭐", f"{v:.2f}%" if not np.isnan(v) else "—",
            f"{d:+.0f}bp" if not np.isnan(d) else None,
            delta_color="inverse", help="THE indicator. Crypto's discount rate.")
v, d = yld("US10Y")
k[1].metric("10Y", f"{v:.2f}%" if not np.isnan(v) else "—",
            f"{d:+.0f}bp" if not np.isnan(d) else None, delta_color="inverse")

for i, (name, key) in enumerate([("DXY", "DXY"), ("Brent", "BRENT"),
                                 ("Gold", "GOLD"), ("VIX", "VIX")], start=2):
    s = h.get(C.TK[key], pd.Series(dtype=float)).dropna()
    if len(s) > 1:
        k[i].metric(name, f"{s.iloc[-1]:,.2f}",
...
for i, sym in enumerate(["BTC", "ETH", "HYPE"]):
    ...
    with cc[i]:
        st.metric(sym, f"${px:,.2f}" if px else "—",
                  f"{d_.get('chg'):+.2f}%" if d_.get("chg") is not None else None)
...
with a:
    ...
    for kk, w in dur["weights"].items():
        val = dur["components"].get(kk, np.nan)
        st.progress(0 if np.isnan(val) else val,
...
with b:
    ...
    for kk, w in hor["weights"].items():
        val = hor["components"].get(kk, np.nan)
        st.progress(0 if np.isnan(val) else val,
                    text=f"**{kk}** ({int(w*100)}%) — {hor['notes'][kk]}")

# ------------------------------------------------------------------ 30Y chart
st.subheader("30Y yield — the spine")
y30 = h.get(C.TK["US30Y"], pd.Series(dtype=float)).dropna() / 10
if len(y30):
    f = go.Figure()
    f.add_trace(go.Scatter(x=y30.index, y=y30.values, name="30Y",
                           line=dict(width=2)))
    f.add_hline(y=5.22, line_dash="dot",
                annotation_text="5.22 post-Warsh wall")
