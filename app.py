...
# --- version guard: catches a half-updated deploy (old core.py + new pages)
if getattr(C, "VERSION", 1) < 2:
    st.error(
        "### ⚠️ `core.py` is out of date\n"
        "The pages were updated but `core.py` on this deploy is the old "
        "version, so the app can't start.\n\n"
        "**Fix:** replace `core.py` in the repo root with the current version "
        "and push again. Everything else is already correct.")
    st.stop()
